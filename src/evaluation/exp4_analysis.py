...
Rebuilt from src/evaluation/__pycache__/exp4_analysis.cpython-313.pyc and
verified against it by scripts/pubs/verify_exp4_reconstruction.py.
"""
...
def analyze_exp4(manifest_path: Path) -> Dict[str, Any]:
    ...
    (output_dir / "thesis_fragment_es.qmd").write_text(
        _thesis_fragment_es(data, outputs), encoding="utf-8"
    )
...
def _reliability(data: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, group in data.groupby(group_cols, dropna=False):
        ...
        for field in SCORE_FIELDS:
            ...
            row[f"{field}_range_across_replicates"] = (
                group[f"{field}_score"].max() - group[f"{field}_score"].min()
            )
        rows.append(row)
    return pd.DataFrame(rows)
...
def _bias_diagnostics(data: pd.DataFrame) -> pd.DataFrame:
    ...
    if primary.empty or visible.empty:
        return pd.DataFrame(
            [{"note": "bias diagnostics require hidden-label and label-visible conditions"}]
        )
...
def _thesis_fragment_es(data: pd.DataFrame, outputs: Dict[str, pd.DataFrame]) -> str:
    return (
        "EXP4 incorpora evaluadores basados en LLM como una medida semantica proxy para examinar "
        "claridad, completitud, concision, plausibilidad semantica, utilidad de auditoria, "
        "accionabilidad y calidad global de las explicaciones. El paquete analizado contiene "
        f"{len(data)} juicios sobre {data['case_id'].nunique()} casos. La evidencia se interpreta "
        "como complemento metodologico de los experimentos tecnicos, no como sustituto de "
        "validacion con participantes humanos.\n"
    )
