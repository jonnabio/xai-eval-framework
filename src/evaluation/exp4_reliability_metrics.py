...
The original module was lost from the working tree; this file was rebuilt from
the committed bytecode (src/evaluation/__pycache__/*.cpython-313.pyc) and is
verified against it by scripts/pubs/verify_exp4_reconstruction.py, which
compares the opcode stream of this file against the original .pyc. Behaviour is
equivalent; comment placement and internal formatting are not original.
"""
...
class ICC:
    ...
    def icc_2_1(
        ...
        pivot = data.pivot_table(
            index=case_col, columns=judge_col, values=score_col, aggfunc="mean"
        )
    ...
    def icc_per_dimension(
        ...
        for dim in dimensions:
            ...
            results.append({"dimension": dim, **icc_dict})
...
class KrippendorffAlpha:
    ...
    def alpha_per_dimension(
        ...
        for dim in dimensions:
            ...
            results.append({"dimension": dim, **alpha_dict})
...
class MultiJudgeComparison:
    ...
    def judge_disagreement_matrix(
        ...
        case_disagreement = case_disagreement.sort_values(
            "avg_disagreement_std", ascending=False
        )
