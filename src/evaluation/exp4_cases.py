...
Rebuilt from src/evaluation/__pycache__/exp4_cases.cpython-313.pyc and
...
def build_case_inventory(manifest_path: Path, target_cases: Optional[int] = None, seed: Optional[int] = None) -> Dict[str, Any]:
    ...
    return write_case_inventory(
        selected, candidates, manifest.paths.cases_dir, target, random_seed
    )
...
def _cases_from_results_file(result_path: Path, source_experiment: str) -> List[Exp4Case]:
    ...
    explainer = str(metadata.get("explainer_method") or _infer_explainer(result_path)).lower()
