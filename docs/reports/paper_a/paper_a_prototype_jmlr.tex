...
\subsection{Overview of Evaluation Setup (Minimum Necessary Context)}
...
\texttt{[TO FILL: metric definitions or explicit cross-reference to Section 3.6.6]}.
...
\texttt{[TO FILL: post-hoc method and correction]}.
...
\begin{itemize}[leftmargin=*]
    \item \texttt{[TO FILL: parameterization details / failure analysis for Anchors stability near zero]}.
    \item \texttt{[TO FILL: external validation on additional datasets/tasks before generalizing method rankings]}.
\end{itemize}

\section{Validity and Reporting Caveats}
\subsection{Statistical Conclusion Validity}
\textbf{Issue.} EXP2 robustness coverage is incomplete: 50 runs are missing from
the planned 300-run cohort, and one artifact is malformed.

\textbf{Why it matters.} Reduced and potentially selective coverage can affect
confidence in method rankings and the reported quality--cost frontier.

\textbf{Mitigation already done.} Analyses use explicit artifact qualification,
exclude malformed/empty artifacts, enforce block-complete omnibus inference, and
enforce matched-cell constraints for paired tests.

\textbf{Residual risk.}
\texttt{[TO FILL: missingness diagnosis by method/model/seed/sample-size cell and impact analysis]}.

\subsection{Construct Validity}
\textbf{Issue.} Metric labels are implementation-bound; Cost is currently
wall-clock time rather than an energy-normalized measure.

\textbf{Why it matters.} Cross-study comparability depends on exact operational
definitions.

\textbf{Mitigation already done.} Metric computation is code-level and
consistent within the benchmark pipeline.

\textbf{Residual risk.}
\texttt{[TO FILL: metric definitions location/cross-reference for reviewer traceability]}.
EEU is not computed in current EXP2 runtime.

\subsection{Internal Validity (Runtime Comparability and Configuration Fidelity)}
\textbf{Issue.} Runtime and quality comparisons depend on implementation pathway
choices (e.g., TreeSHAP vs KernelSHAP and runtime parameter bindings).

\textbf{Why it matters.} Method differences may partially reflect implementation
choices rather than only conceptual algorithmic differences.

\textbf{Mitigation already done.} Matched-cell controls keep model artifact,
feature space, split/sampling seeds, and metric engine fixed; effective runtime
bindings are disclosed.

\textbf{Residual risk.}
\texttt{[TO FILL: environment spec for publication artifact, including CPU policy and dependency lock granularity]}.
Runtime should be reported under standardized constraints.
\texttt{[TO FILL: standardized runtime constraint protocol (cores, memory cap, timeout policy)]}.

\subsection{External Validity}
\textbf{Issue.} Evidence is benchmark-specific and pairwise contrasts are
restricted to shared-family subsets.

\textbf{Why it matters.} Reported rankings may change across datasets,
modalities, and tasks.

\textbf{Mitigation already done.} The current benchmark includes multiple model
families, explainers, seeds, and sampling intensities.

\textbf{Residual risk.} Generalization beyond the present benchmark setting is
not established.

\subsection{Reproducibility and Operational Validity}
\textbf{Issue.} Strong reproducibility claims require both deterministic
execution and durable archival packaging.

\textbf{Why it matters.} Independent verification depends on traceable lineage
and re-executable artifacts.

\textbf{Mitigation already done.} FOM-7 stage gates, audit outputs, and
deterministic inference exports are provided.

\textbf{Residual risk.}
\texttt{[TO FILL: DOI-backed artifact bundle with versioned lineage map]}.

\subsection{Reporting Scope Boundary: Semantic Evaluation}
\textbf{Issue.} Semantic/user-centric evaluation is outside Paper A scope.

\textbf{Why it matters.} Technical metric superiority does not directly imply
user-perceived usefulness or decision quality.

\textbf{Mitigation already done.} Paper A restricts claims to quantitative
benchmark evidence and does not claim user-centric validity.

\textbf{Residual risk.} A complete semantic layer requires calibrated rubric,
task definitions, and human-study protocol.
\texttt{[TO FILL: future study design pointer for semantic evaluation integration]}.

\section{JMLR-Track Positioning}
This work is positioned for the \textbf{Datasets and Benchmarks} track.
\begin{itemize}[leftmargin=*]
    \item \textbf{Claim:} reusable XAI benchmark artifact stack.
    \textbf{Evidence/artifact:} code, experiment configs, run artifacts, summary exports, and inferential outputs under versioned paths.
    \textbf{Why it matters:} benchmark-track value depends on reusable resources, not only narrative findings.
    \item \textbf{Claim:} auditable failure accounting is integral to the benchmark.
    \textbf{Evidence/artifact:} explicit missing/malformed artifact reporting, integrity audits, and stage-gated FOM-7 progression.
    \textbf{Why it matters:} transparent failure accounting improves trust in benchmark conclusions.
    \item \textbf{Claim:} protocol supports multi-objective method comparison.
    \textbf{Evidence/artifact:} joint reporting of Fidelity, Stability, Sparsity, Faithfulness Gap, and Cost with omnibus and paired non-parametric tests.
    \textbf{Why it matters:} benchmark utility increases when quality, robustness, and compute trade-offs are jointly measurable.
    \item \textbf{Claim:} operational novelty is provided by FOM-7.
    \textbf{Evidence/artifact:} explicit stage/gate model linking protocol definition, integrity checks, inference export, and claim-ready reporting.
    \textbf{Why it matters:} contributes an auditable benchmarking method beyond score aggregation.
    \item \textbf{Claim:} novelty relative to prior XAI benchmarks/toolkits is explicit but requires finalized comparative framing.
    \textbf{Evidence/artifact:} current manuscript contains methodological structure and artifact lineage; comparative novelty matrix is pending.
    \textbf{Why it matters:} benchmark-track review requires clear differentiation from prior resources.
    \texttt{[TO FILL: comparison citations and novelty delta statement]}.
\end{itemize}
Artifact package statement for submission:
\begin{itemize}[leftmargin=*]
    \item code/configs/analysis outputs/failure reports are present,
    \item logs and lineage graph require indexed packaging metadata.
    \texttt{[TO FILL: artifact index with paths for logs/lineage graph]}.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Next steps:} finalize missingness diagnostics and archival package.
    \texttt{[TO FILL: missingness diagnosis + DOI package plan]}.
    \item \textbf{Next steps:} integrate deferred semantic evaluation via calibrated rubric and task-grounded human study.
    \texttt{[TO FILL: future semantic study protocol]}.
\end{itemize}
