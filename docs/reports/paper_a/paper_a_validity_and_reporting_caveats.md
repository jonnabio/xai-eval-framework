...
**Issue.** The committed `experiments/exp2_scaled/results` tree still contains 34 unavailable artifact cells from the planned 300-run grid: 9 are missing folders and 25 are present-but-empty result artifacts. The current merged recovery analysis overlays 30 SHAP reruns from `outputs/batch_results.csv`, which reduces the unresolved gap to 32 unavailable unique cells.
...
**Mitigation already done.** The analysis uses explicit artifact qualification, excludes empty artifact-tree runs, applies block-complete filtering for omnibus tests, and uses matched-cell filtering for paired tests. In the current merged recovery snapshot, this combines 291 committed result artifacts with a 30-row SHAP recovery batch, replacing 28 existing `mlp_shap`/`svm_shap` runs and filling 2 previously unavailable SHAP cells. The resulting evidence set contains 268 analyzable unique runs out of the planned 300 (89.3% analyzable coverage).

**Residual risk / Systematic Diagnosis.** Availability loss remains systematic after the recovery overlay:

1. **Residual unavailable cells (32 cells):** After the SHAP recovery overlay, unresolved availability loss is concentrated in **Anchors (24)** and **DiCE (8)**. SHAP is fully covered in the merged snapshot through the combination of committed artifacts and the recovery batch, and LIME remains fully covered at 75/75.
2. **Residual missing result cells (7 cells):** These remaining missing cells are confined to Anchors and DiCE families. This means the grid is substantially more complete than the prior March 2026 submission snapshot, but it still falls short of full factor coverage.
...
Omnibus results remain valid as the filtering protocol still admits model-size blocks with 100% explainer family coverage (15/15 blocks), but the effective merged dataset is still reduced to 268 analyzed runs. Because unresolved unavailable cells are concentrated in Anchors and DiCE, their global quality/cost summaries should be interpreted cautiously even though the SHAP availability problem has been materially reduced by the recovery batch.
