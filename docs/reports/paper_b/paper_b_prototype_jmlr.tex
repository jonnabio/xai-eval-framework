...
\jmlrheading{0}{2026}{1-\pageref{LastPage}}{Submitted 3/2026}{Preprint}{PAPER-B-0001}{Jonathan Herrera-Vasquez}
\ShortHeadings{LIME vs SHAP: A Paired Comparison}{Herrera-Vasquez}
\firstpageno{1}

\begin{document}

\title{LIME vs SHAP: A Paired Empirical Comparison}
...
\begin{abstract}
\input{../../../pub/fragments/paper_b_abstract_en.tex}
\end{abstract}

\begin{keywords}
\input{../../../pub/fragments/paper_b_keywords_en.tex}
\end{keywords}
...
The SHAP--LIME differences reported here should be interpreted as comparative
performance within our benchmark protocol, not as definitive evidence that one
method is universally more correct. Recent formal analyses show that multiple
attribution families can produce high-importance assignments to suppressor
variables in dependent-feature settings, which complicates direct interpretation
of attribution magnitude as target-relevant importance
\citep{wilming2022scrutinizing,wilming2023theoretical,haufe2026formalization}.
Consequently, observed performance gaps between SHAP and LIME may reflect both
methodological differences and sensitivity to data dependence structure.

To reduce over-interpretation, we distinguish \emph{ranking claims} from
\emph{validity claims}. A ranking claim states that one explainer outperforms
another on predefined benchmark criteria in this dataset and protocol. A validity
claim would require stronger evidence that explanations satisfy a formal
correctness criterion for the intended task, potentially including controlled
ground-truth tests and explicit assumption checks
\citep{haufe2026formalization,doshivelez2017rigorous}. In this paper, we
primarily make ranking claims and treat broader validity claims as provisional.

This framing also aligns with prior findings that faithfulness-style evaluations
can be informative but are not always sufficient to resolve correctness questions
in isolation \citep{jacovi2020faithfulness,adebayo2018sanity,hooker2019benchmark,
rong2022consistent}. Therefore, the SHAP--LIME comparison presented here is best
read as robust comparative evidence under the evaluated conditions, with external
validity depending on how closely downstream applications match those conditions.
...
\subsection{EXP3 Context for External Validity}
...
human-grounded usefulness or semantic clarity at the primary benchmark level.
A companion LLM-as-a-judge evaluation (EXP4) has been completed: 4,775
structured judgments from three GPT-4.1 variants on 192 cases show that SHAP's
technical quality advantage extends to semantic dimensions, with moderate
inter-judge agreement on completeness ($\text{ICC}(2,1)=0.585$) and semantic
plausibility (0.601). Human-calibrated validation remains ongoing; the proxy
semantics are currently exploratory.
