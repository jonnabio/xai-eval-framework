...
benchmarking protocol. Across 75 matched SHAP-LIME cells in the Adult
benchmark, SHAP dominates on stability, fidelity, and faithfulness gap,
while LIME yields systematically sparser explanations and lower runtime
in most contexts. The cost trade-off is statistically significant but
...
3,660.7 ms because kernel-SHAP tails are heavy. A downstream EXP3
validation package does not extend the paired SHAP-LIME test because
LIME is absent by design, but it supports the portability of the SHAP
quality signal against Anchors across Breast Cancer and German Credit.
These findings support objective-driven deployment guidance: SHAP for
high-assurance auditing and LIME for latency-sensitive user-facing
...
EXP3 is excluded from the Paper B paired design by construction. It was
designed as a compact external-validity package on Breast Cancer and German
Credit with RF/XGB, SHAP/Anchors, three seeds, and \(N=100\). Because LIME is
not part of that matrix, EXP3 cannot contribute observations to the
SHAP-LIME Wilcoxon tests. Its methodological role in this paper is limited to
external-validity context after the EXP2 paired inference has been reported.
...
contain 275 analyzable unique runs out of 300 planned cells overall.
...
### EXP3 Context for External Validity

The repository now includes a completed EXP3 cross-dataset validation package on
Breast Cancer and German Credit. This package should be treated as supporting
context for Paper B, not as a replacement for the SHAP-LIME paired test,
because EXP3 compares SHAP with Anchors rather than LIME. The useful conclusion
is narrower: SHAP's technical-quality profile persists when evaluated against a
compact rule-style explainer on two additional tabular datasets.

Across 24 EXP3 runs, SHAP has higher mean fidelity (0.702 vs. 0.319) and
stability (0.873 vs. 0.533) than Anchors, while Anchors remains much more
compact (active-feature ratio 0.080 vs. 0.756). EXP3 also shows why runtime
language must remain bounded: in this validation package, Anchors is slower than
SHAP on average. Paper B should therefore continue to claim a LIME-SHAP
quality-cost frontier under EXP2, while using EXP3 only to show that the broader
technical-quality pattern is not obviously Adult-specific.
...
recovery snapshot contains 275 analyzable unique runs overall, but the
...
EXP3 provides supporting evidence from two additional tabular datasets, but image
and text domains may exhibit different trade-offs.
...
integration of thesis-planned causal or counterfactual endpoints remain open
follow-up tasks. EXP3 is the first completed tabular extension, not a universal
