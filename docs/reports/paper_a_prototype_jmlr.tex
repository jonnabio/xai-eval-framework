...
\subsection{Literature-Grounded Design Rationale (Paper A Scope)}
...
\subsection{Experimental Cohorts and Scope (EXP1 vs EXP2)}
We separate evidence into four cohorts:
\begin{itemize}[leftmargin=*]
    \item \textbf{EXP1 pilot/calibration} (\texttt{experiments/exp1\_adult/results})
    \item \textbf{EXP1 reproducibility cohort} (\texttt{experiments/exp1\_adult/reproducibility})
    \item \textbf{EXP2 comparative cohort} (\texttt{experiments/exp2\_comparative/results})
    \item \textbf{EXP2 robustness cohort} (\texttt{experiments/exp2\_scaled/results})
\end{itemize}
...
The benchmark uses UCI Adult via \texttt{src/data\_loading/adult.py} with:
\begin{itemize}[leftmargin=*]
    ...
    \item Categorical pipeline: \texttt{OneHotEncoder(handle\_unknown='ignore', sparse\_output=False)}
...
\begin{itemize}[leftmargin=*]
    \item \texttt{experiments/exp1\_adult/results}: 29 files (4 core, 24 tuning, 1 diagnostic)
    \item \texttt{experiments/exp1\_adult/reproducibility}: 36 core repeated runs (+1 excluded test run)
    \item \texttt{experiments/exp2\_comparative/results}: 20 files (complete $5\times4$ matrix)
    \item \texttt{experiments/exp2\_scaled/results}: 250 files (of 300 planned)
    \item One malformed JSON: \texttt{experiments/exp2\_scaled/results/svm\_shap/seed\_999/n\_200/results.json}
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \texttt{python3 scripts/run\_reproducibility\_study.py --config-dir configs/experiments --pattern "exp1\_adult\_*.yaml" --output-dir experiments/exp1\_adult/reproducibility}
\end{enumerate}

\section{Results}

\subsection{Global Trade-offs (EXP2 robustness; 15 complete model-size blocks)}
\begin{table}[h]
\centering
